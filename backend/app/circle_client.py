...
Uses Circle's Programmable Wallets API (v1) to:
1. Create wallets for the agent and users
2. Execute USDC transfers on Arc testnet
3. Query wallet balances

Docs: https://developers.circle.com/w3s/reference
Arc testnet chain: configured via ARC CLI or manual RPC.
"""
...
class CircleClient:
    ...
    def get_usdc_amount(self, balance_info: Optional[dict]) -> float:
        """Extract the USDC amount (as float) from a get_wallet_balance response."""
        if not balance_info:
            return 0.0
        for b in balance_info.get("balances", []):
            # Circle SDK returns tokenBalance with a nested token object
            token = b.get("token", {})
    ...
    async def create_wallet_set(self, name: str = "AlphaOracle Agent") -> Optional[dict]:
        ...
        try:
            ...
            logger.info(json.loads(wallet_set.model_dump_json()))
            ...
            #             "name": name,
            #             "entitySecretCiphertext": self._entity_secret,
            #         },
            #     )
            #     resp.raise_for_status()
            #     data = resp.json()
            #     ws = data.get("data", {}).get("walletSet", {})
            #     logger.info("Created wallet set: %s", ws.get("id"))
            #     return ws
    ...
    async def create_wallet(
        ...
        """Create one or more wallets within a wallet set."""
        if not self._enabled:
            return [
                {
    ...
    async def get_wallet_balance(self, wallet_id: str) -> Optional[dict]:
        ...
        if not self._enabled:
            # Return empty balances — bankroll comes from DEFAULT_BANKROLL when Circle is off
            return {
                "wallet_id": wallet_id,
                "balances": [],
                "mock": True,
    ...
    async def transfer_usdc(
        ...
        This is used when the agent executes a trade (simulated in testnet).
        """
        if not self._enabled:
            tx_hash = f"0x{uuid.uuid4().hex}"
            logger.info(
                "Mock USDC transfer: %s USDC from %s to %s (tx: %s)",
                amount, from_wallet_id, to_address, tx_hash,
            )
            return {
                "id": str(uuid.uuid4()),
                "state": "COMPLETE",
        ...
        try:
            ...
            logger.info(f"transfer response:\n{json.dumps(json.loads(transfer_response.model_dump_json()), indent=2)}")
            ...
            while current_state not in terminal_states:
                ...
                logger.info(f"Poll response: {json.dumps(json.loads(poll_response.model_dump_json()), indent=2)}")
            ...
            #             "destinationAddress": to_address,
            #             "amounts": [amount],
            #             "tokenId": self._usdc_token_id(blockchain),
            #             "blockchain": blockchain,
            #             "feeLevel": "MEDIUM",
            #         },
            #     )
            ...
            #     data = resp.json()
            #     tx = data.get("data", {})
            #     logger.info("Transfer submitted: %s", tx.get("id"))
            #     return tx
        except Exception as exc:
    ...
    def _usdc_token_id(self, blockchain: str) -> str:
        """Return the USDC token ID for the given blockchain.

        These are Circle-specific identifiers.
        For Arc testnet, this will need to be updated with the actual token ID
        from the Arc developer docs.
        """
        # Placeholder — replace with actual Arc testnet USDC token ID
        token_map = {
