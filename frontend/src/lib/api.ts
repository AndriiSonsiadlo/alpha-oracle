...
export const api = {
  ...
  getPortfolioHistory: (limit = 500) => apiFetch<EquityPoint[]>(`/api/portfolio/history?limit=${limit}`),
  getPositions: (status = "open") => apiFetch<Position[]>(`/api/positions?status=${status}`),
  ...
  createStrategy: (config: StrategyConfig, description = "", label?: string) =>
    apiFetch<StrategyVersion>(
      `/api/strategies?description=${encodeURIComponent(description)}${label ? `&label=${encodeURIComponent(label)}` : ""}`,
      { method: "POST", body: JSON.stringify(config) }
    ),
  rollbackStrategy: (versionId: string) =>
    apiFetch<StrategyVersion>(`/api/strategies/${versionId}/rollback`, { method: "POST" }),
  ...
  setupWallet: () =>
    apiFetch<{ wallet_set: Record<string, string>; wallets: Record<string, string>[]; circle_enabled: boolean }>(
      "/api/wallet/setup", { method: "POST" }
    ),
  connectWallet: (walletId: string) =>
    apiFetch<WalletConnectResult>(`/api/wallet/connect?wallet_id=${encodeURIComponent(walletId)}`, { method: "POST" }),
  syncWalletBalance: () => apiFetch<WalletSyncResult>("/api/wallet/sync-balance", { method: "POST" }),
  getWalletStatus: () => apiFetch<WalletStatus>("/api/wallet/status"),
  resetSession: (mode: "demo" | "live") =>
    apiFetch<{ mode: string; bankroll: number; status: string }>(
      `/api/session/reset?mode=${mode}`, { method: "POST" }
    ),
